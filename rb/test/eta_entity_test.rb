...
class EtaEntityTest < Minitest::Test
  ...
  def test_basic_flow
    ...
    if setup[:synthetic_only]
      skip "live entity test uses synthetic IDs from fixture — set REAL_TIME_BUS_DATA_TEST_ETA_ENTID JSON to run live"
      return
    end
    client = setup[:client]
    ...
    eta_ref01_data = nil
    if eta_ref01_data_raw.length > 0
...
def eta_basic_setup(extra)
  Runner.load_env_local

  entity_data_file = File.join(__dir__, "..", "..", ".sdk", "test", "entity", "eta", "EtaTestData.json")
  ...
  entid_env_raw = ENV["REAL_TIME_BUS_DATA_TEST_ETA_ENTID"]
