...
class RouteStopDirectTest < Minitest::Test
  def test_direct_list_route_stop
    setup = route_stop_direct_setup([
      { "id" => "direct01" },
      { "id" => "direct02" },
    ])
    _should_skip, _reason = Runner.is_control_skipped("direct", "direct-list-route_stop", setup[:live] ? "live" : "unit")
    if _should_skip
  ...
  def test_direct_load_route_stop
    setup = route_stop_direct_setup({ "id" => "direct01" })
    _should_skip, _reason = Runner.is_control_skipped("direct", "direct-load-route_stop", setup[:live] ? "live" : "unit")
    if _should_skip
      skip(_reason || "skipped via sdk-test-control.json")
      return
    end
...
def route_stop_direct_setup(mockres)
