...
class RouteEntityTest < Minitest::Test
  ...
  def test_stream
    ...
    seen = base.Route(nil).stream("list", nil, nil).to_a
    ...
    if cfg["feature"].is_a?(Hash) && cfg["feature"].key?("streaming")
      ...
      got = []
      sdk.Route(nil).stream("list", nil, nil).each do |item|
        if item.is_a?(Array)
          got.concat(item)
        else
          got << item
  ...
  def test_basic_flow
    ...
    if setup[:synthetic_only]
      skip "live entity test uses synthetic IDs from fixture — set REAL_TIME_BUS_DATA_TEST_ROUTE_ENTID JSON to run live"
      return
    end
    client = setup[:client]
    ...
    route_ref01_data_dt0_load_result = Helpers.to_map(route_ref01_data_dt0_loaded.respond_to?(:data_get) ? route_ref01_data_dt0_loaded.data_get : route_ref01_data_dt0_loaded)
    assert !route_ref01_data_dt0_load_result.nil?
    assert_equal route_ref01_data_dt0_load_result["id"], route_ref01_data["id"]
...
def route_basic_setup(extra)
  ...
  entid_env_raw = ENV["REAL_TIME_BUS_DATA_TEST_ROUTE_ENTID"]
