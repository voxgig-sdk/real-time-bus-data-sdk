# RealTimeBusData Ruby SDK

The Ruby SDK for the RealTimeBusData API. Provides an entity-oriented interface using idiomatic Ruby conventions.
...
```ruby
result, err = client.Eta(nil).load({ "id" => "example_id" }, nil)
raise err if err
puts result
```
...
result, err = client.RealTimeBusData(nil).load(
  { "id" => "test01" }, nil
)
...
```
rb/
├── RealTimeBusData_sdk.rb       -- Main SDK module
