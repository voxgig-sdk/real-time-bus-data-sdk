...
class RouteEntity
  ...
  def make
    opts = @_entopts.dup
    RouteEntity.new(@_client, opts)
  end

  def data_set(args)
    if args
      @_data = RealTimeBusDataHelpers.to_map(VoxgigStruct.clone(args)) || {}
      @_utility.feature_hook.call(@_entctx, "SetData")
    end
  end

  # @return [Route, Hash] the current Route data
  def data_get
  ...
  def match_set(args)
    if args
      @_match = RealTimeBusDataHelpers.to_map(VoxgigStruct.clone(args)) || {}
      @_utility.feature_hook.call(@_entctx, "SetMatch")
    end
  end

  # @return [Hash] the current match filter (any subset of Route fields)
  def match_get
  ...
  # Load a single Route.
  #
  # @param reqmatch [RouteLoadMatch, Hash, nil] match criteria (id/query fields);
  #   optional — an entity with no id-like key loads with no match (nil is treated
  #   as an empty match, so client.Route.load works with no args).
  # @param ctrl [Object, nil] optional per-call control
  # @return [Route, Hash] the loaded Route; raises RealTimeBusDataError on failure
  ...
  #
  # @param reqmatch [RouteListMatch, Hash, nil] match filter (any subset of
  #   Route fields); defaults to nil, treated as an empty match that lists all.
  # @param ctrl [Object, nil] optional per-call control
  # @return [Array<Route>, Array] the matching Route items; raises RealTimeBusDataError on failure
