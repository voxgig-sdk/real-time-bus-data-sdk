...
class TestRouteStopDirect:

    def test_should_direct_list_route_stop(self):
        setup = _route_stop_direct_setup([
            {"id": "direct01"},
            {"id": "direct02"},
        ])
        _skip, _reason = runner.is_control_skipped("direct", "direct-list-route_stop", "live" if setup["live"] else "unit")
        if _skip:
...
def _route_stop_direct_setup(mockres):
