...
class TestRouteStopEntity:

    def test_should_create_instance(self):
        testsdk = RealTimeBusDataSDK.test(None, None)
        ent = testsdk.RouteStop(None)
    ...
    def test_should_stream(self):
        ...
        seen = list(base.RouteStop(None).stream("list", None, None))
        assert len(seen) == 3
        ...
        if isinstance(cfg.get("feature"), dict) and "streaming" in cfg["feature"]:
            ...
            got = []
            for item in sdk.RouteStop(None).stream("list", None, None):
    ...
    def test_should_run_basic_flow(self):
        setup = _route_stop_basic_setup(None)
        ...
        if setup.get("synthetic_only"):
            pytest.skip("live entity test uses synthetic IDs from fixture — "
                        "set REAL_TIME_BUS_DATA_TEST_ROUTE_STOP_ENTID JSON to run live")
        client = setup["client"]

        # Bootstrap entity data from existing test data.
        route_stop_ref01_data_raw = vs.items(helpers.to_map(
            vs.getpath(setup["data"], "existing.route_stop")))
        route_stop_ref01_data = None
        if len(route_stop_ref01_data_raw) > 0:
...
def _route_stop_basic_setup(extra):
    ...
    entity_data_file = os.path.join(_TEST_DIR, "../../.sdk/test/entity/route_stop/RouteStopTestData.json")
    ...
    idmap_resolved = helpers.to_map(
        env.get("REAL_TIME_BUS_DATA_TEST_ROUTE_STOP_ENTID"))
