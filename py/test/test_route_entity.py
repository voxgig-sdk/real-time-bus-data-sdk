...
class TestRouteEntity:
    ...
    def test_should_stream(self):
        ...
        seen = list(base.Route(None).stream("list", None, None))
        assert len(seen) == 3
        ...
        if isinstance(cfg.get("feature"), dict) and "streaming" in cfg["feature"]:
            ...
            got = []
            for item in sdk.Route(None).stream("list", None, None):
    ...
    def test_should_run_basic_flow(self):
        setup = _route_basic_setup(None)
        # Per-op sdk-test-control.json skip — basic test exercises a flow with
        # multiple ops; skipping any one skips the whole flow (steps depend
        # on each other).
        _live = setup.get("live", False)
        for _op in ["list", "load"]:
            _skip, _reason = runner.is_control_skipped("entityOp", "route." + _op, "live" if _live else "unit")
            if _skip:
        ...
        if setup.get("synthetic_only"):
            pytest.skip("live entity test uses synthetic IDs from fixture — "
                        "set REAL_TIME_BUS_DATA_TEST_ROUTE_ENTID JSON to run live")
        client = setup["client"]

        # Bootstrap entity data from existing test data.
        route_ref01_data_raw = vs.items(helpers.to_map(
            vs.getpath(setup["data"], "existing.route")))
        route_ref01_data = None
        if len(route_ref01_data_raw) > 0:
        ...
        route_ref01_data_dt0_load_result = helpers.to_map(runner.entity_data(route_ref01_data_dt0_loaded))
        assert route_ref01_data_dt0_load_result is not None
        assert route_ref01_data_dt0_load_result["id"] == route_ref01_data["id"]



def _route_basic_setup(extra):
    runner.load_env_local()

    entity_data_file = os.path.join(_TEST_DIR, "../../.sdk/test/entity/route/RouteTestData.json")
    ...
    idmap_resolved = helpers.to_map(
        env.get("REAL_TIME_BUS_DATA_TEST_ROUTE_ENTID"))
    if idmap_resolved is None:
        idmap_resolved = helpers.to_map(idmap)

    if env.get("REAL_TIME_BUS_DATA_TEST_LIVE") == "TRUE":
    ...
    _live = env.get("REAL_TIME_BUS_DATA_TEST_LIVE") == "TRUE"
    return {
        "client": client,
        "data": entity_data,
        "idmap": idmap_resolved,
        "env": env,
        "explain": env.get("REAL_TIME_BUS_DATA_TEST_EXPLAIN") == "TRUE",
        "live": _live,
