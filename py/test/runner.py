# RealTimeBusData SDK test runner
...
class RealTimeBusDataTestRunner:
    _env = {}
    ...
    def load_env_local():
        ...
        for line in content.splitlines():
            ...
            RealTimeBusDataTestRunner._env[key] = val

    @staticmethod
    def getenv(key):
        val = RealTimeBusDataTestRunner._env.get(key)
        if val is not None:
            return val
    ...
    @staticmethod
    def env_override(m):
        live = RealTimeBusDataTestRunner.getenv("REAL_TIME_BUS_DATA_TEST_LIVE")
        override = RealTimeBusDataTestRunner.getenv("REAL_TIME_BUS_DATA_TEST_OVERRIDE")

        if live == "TRUE" or override == "TRUE":
            for key in list(m.keys()):
                envval = RealTimeBusDataTestRunner.getenv(key)
        ...
        explain = RealTimeBusDataTestRunner.getenv("REAL_TIME_BUS_DATA_TEST_EXPLAIN")
    ...
    def load_test_control():
        ...
        """
        if RealTimeBusDataTestRunner._test_control is not None:
            return RealTimeBusDataTestRunner._test_control
        ctrl_path = os.path.join(os.path.dirname(__file__), "sdk-test-control.json")
        try:
            with open(ctrl_path, "r") as f:
                RealTimeBusDataTestRunner._test_control = json.load(f)
        except (FileNotFoundError, IOError, ValueError):
            RealTimeBusDataTestRunner._test_control = {
                "version": 1,
                "test": {"skip": {
    ...
    def is_control_skipped(kind, name, mode):
        ...
        ctrl = RealTimeBusDataTestRunner.load_test_control()
...
def load_env_local():
    RealTimeBusDataTestRunner.load_env_local()


def env_override(m):
    return RealTimeBusDataTestRunner.env_override(m)


def entity_data(v):
    return RealTimeBusDataTestRunner.entity_data(v)


def entity_list_to_data(lst):
    return RealTimeBusDataTestRunner.entity_list_to_data(lst)


def is_control_skipped(kind, name, mode):
    return RealTimeBusDataTestRunner.is_control_skipped(kind, name, mode)


def load_test_control():
    return RealTimeBusDataTestRunner.load_test_control()


def live_delay_ms():
    return RealTimeBusDataTestRunner.live_delay_ms()
