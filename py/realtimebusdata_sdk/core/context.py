...
from realtimebusdata_sdk.core.control import RealTimeBusDataControl
...
from realtimebusdata_sdk.core.helpers import get_ctx_prop, to_map
...
class RealTimeBusDataContext:
    def __init__(self, ctxmap=None, basectx=None):
        ...
        # Ctrl
        self.ctrl = RealTimeBusDataControl()
        ...
        if isinstance(sp, RealTimeBusDataSpec):
            self.spec = sp
        elif basectx is not None:
        ...
        if isinstance(r, RealTimeBusDataResult):
            self.result = r
        elif basectx is not None:
        ...
        if isinstance(rp, RealTimeBusDataResponse):
            self.response = rp
        elif basectx is not None:
    ...
    def resolve_op(self, opname):
        ...
        if opname == "":
            return RealTimeBusDataOperation({})
    ...
    def make_error(self, code, msg):
        return RealTimeBusDataError(code, msg, self)
