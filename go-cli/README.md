...
boru-driven command-line client **and** interactive REPL for the RealTimeBusData
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/real-time-bus-data-cli)
make build

# 2. See usage (words, entities, env vars)
./real-time-bus-data-cli --help
...
1. **Build the binary.** From this `go-cli/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/real-time-bus-data-cli
   ```
...
```sh
./real-time-bus-data-cli load 1 eta          # scalar shorthand for {id:1}
./real-time-bus-data-cli load '{id:1}' eta     # explicit match map
```
...
Run with no arguments to open a REPL (prompt `real-time-bus-data>`). Each line is
...
- `<entity>` is a bareword, auto-quoted as an boru atom (e.g. `eta`).
...
| Target | Result |
|--------|--------|
| `make build` | Native binary at `dist/<os>-<arch>/real-time-bus-data-cli`. |
...
(`./real-time-bus-data-cli <expr>`) and interactively (the REPL), and expressions compose the
