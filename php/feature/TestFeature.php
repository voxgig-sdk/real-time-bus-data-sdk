...
// RealTimeBusData SDK test feature

require_once __DIR__ . '/BaseFeature.php';
require_once __DIR__ . '/../utility/Param.php';

class RealTimeBusDataTestFeature extends RealTimeBusDataBaseFeature
{
    ...
    public function init(RealTimeBusDataContext $ctx, array $options): void
    {
        ...
        $test_fetcher = function (RealTimeBusDataContext $fctx, string $_fullurl, array $_fetchdef) use ($entity): array {
            ...
            if ($op->name === 'load') {
                ...
                $id = RealTimeBusDataParam::call($fctx, 'id');
    ...
    public function makeNetsim(array $net, callable $inner): callable
    {
        ...
        return function (RealTimeBusDataContext $fctx, string $url, array $fetchdef) use ($net, $inner, $pick_latency, $sleep): array {
            $this->_netcalls++;
            $call = $this->_netcalls;
     ...
     */
    public function buildArgs(RealTimeBusDataContext $ctx, $op, $args): array
    {
        ...
        foreach ($keys as $k) {
            ...
            if ($is_id || $in_reqd) {
                $v = RealTimeBusDataParam::call($ctx, $k);
