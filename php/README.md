...
The PHP SDK for the RealTimeBusData API. Provides an entity-oriented interface using PHP conventions.
...
```php
[$result, $err] = $client->Eta(null)->list(null, null);
...
```php
[$result, $err] = $client->Eta(null)->load(["id" => "example_id"], null);
if ($err) { throw new \Exception($err); }
print_r($result);
```
...
[$result, $err] = $client->RealTimeBusData(null)->load(
    ["id" => "test01"], null
);
