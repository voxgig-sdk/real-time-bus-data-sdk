...
// RealTimeBusData SDK Route entity
...
class RouteEntity
{
    ...
    {
        $opts = $this->_entopts;
        return new RouteEntity($this->_client, $opts);
    }

    /**
     * @param Route|array $args Route data (assoc-array) to store.
     */
    public function data_set($args): void
    {
        if ($args) {
            $this->_data = RealTimeBusDataHelpers::to_map(Struct::clone($args)) ?? [];
            ($this->_utility->feature_hook)($this->_entctx, "SetData");
        }
    }

    /**
     * @return Route|array The current Route data as an assoc-array.
     */
    ...
    /**
     * @param array $args Match filter (any subset of Route fields).
     */
    public function match_set($args): void
    {
        if ($args) {
            $this->_match = RealTimeBusDataHelpers::to_map(Struct::clone($args)) ?? [];
            ($this->_utility->feature_hook)($this->_entctx, "SetMatch");
        }
    }

    /**
     * @return array The current match filter (any subset of Route fields).
     */
     ...
     * Load a single Route.
     *
     * @param RouteLoadMatch|array|null $reqmatch Match criteria (id/query
     *   fields) as an assoc-array; a typed RouteLoadMatch names the shape.
     * @param mixed $ctrl Optional per-call control overrides.
     * @return Route|array The loaded Route as an assoc-array at the
    ...
    /**
     * List Route items matching the given filter.
     *
     * @param RouteListMatch|array|null $reqmatch Match filter (any subset
     *   of Route fields) as an assoc-array; RouteListMatch names the shape.
     * @param mixed $ctrl Optional per-call control overrides.
     * @return Route[]|array A list of Route items as assoc-arrays at
