...
class RouteEntityTest extends TestCase
{
    public function test_create_instance(): void
    {
        $testsdk = RealTimeBusDataSDK::test(null, null);
        $ent = $testsdk->Route(null);
    ...
    public function test_stream(): void
    {
        ...
        $base = RealTimeBusDataSDK::test($seed, null);
        $seen = iterator_to_array($base->Route(null)->stream("list", null, null), false);
        ...
        if (isset($cfg["feature"]) && is_array($cfg["feature"]) && isset($cfg["feature"]["streaming"])) {
            ...
            $got = [];
            foreach ($sdk->Route(null)->stream("list", null, null) as $item) {
    ...
    {
        $setup = route_basic_setup(null);
        // Per-op sdk-test-control.json skip.
        $_live = !empty($setup["live"]);
        foreach (["list", "load"] as $_op) {
            [$_shouldSkip, $_reason] = Runner::is_control_skipped("entityOp", "route." . $_op, $_live ? "live" : "unit");
            if ($_shouldSkip) {
        ...
        if (!empty($setup["synthetic_only"])) {
            $this->markTestSkipped("live entity test uses synthetic IDs from fixture — set REAL_TIME_BUS_DATA_TEST_ROUTE_ENTID JSON to run live");
            return;
        }
        $client = $setup["client"];
        ...
        $route_ref01_data = null;
        if (count($route_ref01_data_raw) > 0) {
...
function route_basic_setup($extra)
{
    ...
    $entity_data_file = __DIR__ . '/../../.sdk/test/entity/route/RouteTestData.json';
    ...
    // Generate idmap.
    $idmap = [];
    foreach (["route01", "route02", "route03"] as $k) {
    ...
    $entid_env_raw = getenv("REAL_TIME_BUS_DATA_TEST_ROUTE_ENTID");
