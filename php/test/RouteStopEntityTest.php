...
class RouteStopEntityTest extends TestCase
{
    public function test_create_instance(): void
    {
        $testsdk = RealTimeBusDataSDK::test(null, null);
        $ent = $testsdk->RouteStop(null);
    ...
    public function test_stream(): void
    {
        ...
        $seen = iterator_to_array($base->RouteStop(null)->stream("list", null, null), false);
        ...
        if (isset($cfg["feature"]) && is_array($cfg["feature"]) && isset($cfg["feature"]["streaming"])) {
            ...
            $got = [];
            foreach ($sdk->RouteStop(null)->stream("list", null, null) as $item) {
    ...
    public function test_basic_flow(): void
    {
        ...
        if (!empty($setup["synthetic_only"])) {
            $this->markTestSkipped("live entity test uses synthetic IDs from fixture — set REAL_TIME_BUS_DATA_TEST_ROUTE_STOP_ENTID JSON to run live");
            return;
        }
        $client = $setup["client"];
        ...
        $route_stop_ref01_data = null;
        if (count($route_stop_ref01_data_raw) > 0) {
...
function route_stop_basic_setup($extra)
{
    ...
    $entity_data_file = __DIR__ . '/../../.sdk/test/entity/route_stop/RouteStopTestData.json';
    ...
    $entid_env_raw = getenv("REAL_TIME_BUS_DATA_TEST_ROUTE_STOP_ENTID");
