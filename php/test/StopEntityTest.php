...
class StopEntityTest extends TestCase
{
    ...
    public function test_stream(): void
    {
        ...
        $seen = iterator_to_array($base->Stop(null)->stream("list", null, null), false);
        ...
        if (isset($cfg["feature"]) && is_array($cfg["feature"]) && isset($cfg["feature"]["streaming"])) {
            ...
            $got = [];
            foreach ($sdk->Stop(null)->stream("list", null, null) as $item) {
    ...
    public function test_basic_flow(): void
    {
        ...
        if (!empty($setup["synthetic_only"])) {
            $this->markTestSkipped("live entity test uses synthetic IDs from fixture — set REAL_TIME_BUS_DATA_TEST_STOP_ENTID JSON to run live");
            return;
        }
        $client = $setup["client"];
        ...
        $stop_ref01_list_result = $stop_ref01_ent->list($stop_ref01_match, null);
...
function stop_basic_setup($extra)
{
    ...
    $entid_env_raw = getenv("REAL_TIME_BUS_DATA_TEST_STOP_ENTID");
