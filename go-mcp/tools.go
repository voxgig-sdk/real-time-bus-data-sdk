...
type Args struct {
	Entity string         `json:"entity" jsonschema:"eta | route | route_stop | stop"`
	Query  map[string]any `json:"query,omitempty" jsonschema:"optional match map e.g. {\"id\":1} for load, omit for list"`
}

func registerTools(server *mcp.Server, client *sdk.RealTimeBusDataSDK) {
	mcp.AddTool(server, &mcp.Tool{
		Name: "real-time-bus-data_list",
		Description: "List records from RealTimeBusData. " +
	...
	mcp.AddTool(server, &mcp.Tool{
		Name: "real-time-bus-data_load",
		Description: "Load a single record from RealTimeBusData. " +
...
func runOp(client *sdk.RealTimeBusDataSDK, op string, args Args) (*mcp.CallToolResult, any, error) {
...
func entityFor(client *sdk.RealTimeBusDataSDK, name string) (sdk.RealTimeBusDataEntity, error) {
	switch strings.ToLower(name) {
	case "eta":
		return client.Eta(nil), nil
	case "route":
		return client.Route(nil), nil
	case "route_stop":
		return client.RouteStop(nil), nil
	case "stop":
