-- RealTimeBusData SDK RouteStop entity

local vs = require("utility.struct.struct")
local helpers = require("core.helpers")

local RouteStopEntity = {}
RouteStopEntity.__index = RouteStopEntity


function RouteStopEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, RouteStopEntity)
  self._name = "route_stop"
  self._client = client
...
function RouteStopEntity:get_name()
  return self._name
end


function RouteStopEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return RouteStopEntity.new(self._client, opts)
end
...
function RouteStopEntity:mark_deleted()
  self._deleted = true
end


function RouteStopEntity:deleted()
  return true == self._deleted
end


function RouteStopEntity:data_set(args)
  if args ~= nil then
...
function RouteStopEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function RouteStopEntity:match_set(args)
  if args ~= nil then
...
function RouteStopEntity:match_get()
...
function RouteStopEntity:stream(action, args, callopts)
...
---@param reqmatch RouteStopLoadMatch
---@param ctrl? table
---@return RouteStop
---@return string? err
function RouteStopEntity:load(reqmatch, ctrl)
  local utility = self._utility
...
function RouteStopEntity:_run_op(ctx, post_done)
  local utility = self._utility
