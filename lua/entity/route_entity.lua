...
local RouteEntity = {}
RouteEntity.__index = RouteEntity


function RouteEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, RouteEntity)
  self._name = "route"
  self._client = client
...
function RouteEntity:get_name()
  return self._name
end


function RouteEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return RouteEntity.new(self._client, opts)
end
...
function RouteEntity:mark_deleted()
  self._deleted = true
end


function RouteEntity:deleted()
  return true == self._deleted
end


function RouteEntity:data_set(args)
  if args ~= nil then
...
function RouteEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function RouteEntity:match_set(args)
  if args ~= nil then
...
function RouteEntity:match_get()
...
function RouteEntity:stream(action, args, callopts)
...
---@param reqmatch RouteListMatch
---@param ctrl? table
---@return Route[]
---@return string? err
function RouteEntity:list(reqmatch, ctrl)
  local utility = self._utility
  local ctx = utility.make_context({
    opname = "list",
    ctrl = ctrl,
...
function RouteEntity:_run_op(ctx, post_done)
  local utility = self._utility
