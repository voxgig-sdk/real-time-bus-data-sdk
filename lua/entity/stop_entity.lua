...
local StopEntity = {}
StopEntity.__index = StopEntity


function StopEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, StopEntity)
  self._name = "stop"
  self._client = client
...
function StopEntity:get_name()
  return self._name
end


function StopEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return StopEntity.new(self._client, opts)
end
...
function StopEntity:mark_deleted()
  self._deleted = true
end


function StopEntity:deleted()
  return true == self._deleted
end


function StopEntity:data_set(args)
  if args ~= nil then
...
function StopEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function StopEntity:match_set(args)
  if args ~= nil then
...
function StopEntity:stream(action, args, callopts)
...
function StopEntity:_run_op(ctx, post_done)
  local utility = self._utility
