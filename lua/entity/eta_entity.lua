...
local EtaEntity = {}
EtaEntity.__index = EtaEntity


function EtaEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, EtaEntity)
  self._name = "eta"
  self._client = client
...
function EtaEntity:get_name()
  return self._name
end


function EtaEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return EtaEntity.new(self._client, opts)
end
...
function EtaEntity:mark_deleted()
  self._deleted = true
end


function EtaEntity:deleted()
  return true == self._deleted
end


function EtaEntity:data_set(args)
  if args ~= nil then
...
function EtaEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function EtaEntity:match_set(args)
  if args ~= nil then
...
function EtaEntity:stream(action, args, callopts)
...
function EtaEntity:_run_op(ctx, post_done)
  local utility = self._utility
