...
describe("RouteDirect", function()
  it("should direct-list-route", function()
    local setup = route_direct_setup({
      { id = "direct01" },
      { id = "direct02" },
    })
    local _should_skip, _reason = runner.is_control_skipped("direct", "direct-list-route", setup.live and "live" or "unit")
  ...
  it("should direct-load-route", function()
    local setup = route_direct_setup({ id = "direct01" })
    local _should_skip, _reason = runner.is_control_skipped("direct", "direct-load-route", setup.live and "live" or "unit")
    if _should_skip then
      pending(_reason or "skipped via sdk-test-control.json")
      return
    end
...
function route_direct_setup(mockres)
