...
describe("EtaEntity", function()
  it("should create instance", function()
    local testsdk = sdk.test(nil, nil)
    local ent = testsdk:Eta(nil)
  ...
  it("should run basic flow", function()
    ...
    if setup.synthetic_only then
      pending("live entity test uses synthetic IDs from fixture — set REAL_TIME_BUS_DATA_TEST_ETA_ENTID JSON to run live")
      return
    end
    local client = setup.client

    -- Bootstrap entity data from existing test data.
    local eta_ref01_data_raw = vs.items(helpers.to_map(
      vs.getpath(setup.data, "existing.eta")))
    local eta_ref01_data = nil
    if #eta_ref01_data_raw > 0 then
    ...
    assert.is_nil(err)
    assert.is_not_nil(eta_ref01_data_dt0_loaded)
...
function eta_basic_setup(extra)
  ...
  local entity_data_file = _test_dir .. "../../.sdk/test/entity/eta/EtaTestData.json"
  local f = io.open(entity_data_file, "r")
  if f == nil then
    error("failed to read eta test data: " .. entity_data_file)
  end
  ...
  local entid_env_raw = os.getenv("REAL_TIME_BUS_DATA_TEST_ETA_ENTID")
  ...
  local idmap_resolved = helpers.to_map(
    env["REAL_TIME_BUS_DATA_TEST_ETA_ENTID"])
