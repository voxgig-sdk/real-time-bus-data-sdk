...
The Lua SDK for the RealTimeBusData API. Provides an entity-oriented interface using Lua conventions.
...
```lua
local result, err = client:Eta(nil):list(nil, nil)
...
```lua
local result, err = client:Eta(nil):load({ id = "example_id" }, nil)
if err then error(err) end
print(result)
```
...
local result, err = client:RealTimeBusData(nil):load(
  { id = "test01" }, nil
)
