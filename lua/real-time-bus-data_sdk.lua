...
require("real-time-bus-data_types")
...
local RealTimeBusDataSDK = {}
RealTimeBusDataSDK.__index = RealTimeBusDataSDK
...
RealTimeBusDataSDK._make_feature = _make_feature


function RealTimeBusDataSDK.new(options)
  local self = setmetatable({}, RealTimeBusDataSDK)
  self.mode = "live"
  self.features = {}
  self.options = nil
...
function RealTimeBusDataSDK:options_map()
...
function RealTimeBusDataSDK:get_utility()
  return Utility.copy(self._utility)
end


function RealTimeBusDataSDK:get_root_ctx()
  return self._rootctx
end


function RealTimeBusDataSDK:prepare(fetchargs)
...
function RealTimeBusDataSDK:direct(fetchargs)
...
function RealTimeBusDataSDK:_op_allowed(op)
  local allow = vs.getpath(self.options, "allow.op")
  return type(allow) == "string" and allow:find(op, 1, true) ~= nil
end


function RealTimeBusDataSDK:_op_denied(op)
  local allow = vs.getpath(self.options, "allow.op")
  if type(allow) ~= "string" then allow = "" end
  return {
    ok = false,
    err = "RealTimeBusDataSDK: " .. op .. ": operation not allowed by" ..
...
-- by passing it.
function RealTimeBusDataSDK:_raw_request(fetchargs)
  local utility = self._utility
...
function RealTimeBusDataSDK:graphql(query, variables, ctrl)
  ...
  if type(errors) == "table" and 0 < #errors then
    ...
    res.ok = false
    res.err = "RealTimeBusDataSDK: graphql: " .. msg
    res.graphql = errors
  end
...
function RealTimeBusDataSDK.test(testopts, sdkopts)
  sdkopts = sdkopts or {}
  ...
  local sdk = RealTimeBusDataSDK.new(sdkopts)
  sdk.mode = "test"
