...
// RealTimeBusData SDK feature test
//
...
class FtCtrl extends RealTimeBusDataControl
{
...
class FtClient
{
    ...
    private RealTimeBusDataUtility $utility;

    public function __construct(RealTimeBusDataUtility $utility, array $options)
    {
    ...
    public function get_utility(): RealTimeBusDataUtility
    {
        return $this->utility;
    }
}
...
class FtRecorder
{
    ...
    {
        return function (RealTimeBusDataContext $_ctx, string $url, array $fetchdef): array {
...
class FtHarness
{
    ...
    public RealTimeBusDataContext $rootctx;
    ...
    public function __construct(
        ...
        $utility->fetcher = $server ?? function (RealTimeBusDataContext $_ctx, string $_url, array $fetchdef): array {
        ...
        foreach ($features as $fspec) {
            ...
            $f = RealTimeBusDataConfig::make_feature($name);
    ...
    private static function buildUrl(RealTimeBusDataSpec $spec): string
    {
        $keys = [];
    ...
    public function op(array $o = []): array
    {
        ...
        if (($ctx->out['point'] ?? null) instanceof RealTimeBusDataError) {
        ...
        if (is_array($fetched)) {
            $ctx->response = new RealTimeBusDataResponse($fetched);
        }
    ...
    private function fail(RealTimeBusDataContext $ctx, mixed $err): array
    {
...
class FeatureTest extends TestCase
{
    ...
    public function test_retry_retries_a_transport_error_until_exhausted(): void
    {
        ...
        $clock = new FtClock();
        $n = 0;
        $server = function (RealTimeBusDataContext $ctx, string $_url, array $_fd) use (&$n): array {
            $n++;
