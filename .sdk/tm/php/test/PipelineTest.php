...
// RealTimeBusData SDK pipeline test
//
...
{
    private static function utility(): RealTimeBusDataUtility
    {
        return new RealTimeBusDataUtility();
    }
    ...
    private static function ctx(array $over = []): RealTimeBusDataContext
    {
        $utility = $over['utility'] ?? self::utility();
        $client = $over['client'] ?? new PlClient(['base' => 'http://h']);
        $ctx = new RealTimeBusDataContext([
            'client' => $client,
            'utility' => $utility,
        ], null);
        $ctx->op = new RealTimeBusDataOperation(['name' => 'load', 'entity' => 'x']);
        foreach ($over as $k => $v) {
    ...
    {
        return ($err instanceof RealTimeBusDataError) ? $err->sdk_code : '';
    }
    ...
    public function test_make_point_rejects_a_disallowed_operation(): void
    {
        ...
        $ctx->op = new RealTimeBusDataOperation(['name' => 'nope', 'entity' => 'x']);
        [$point, $err] = RealTimeBusDataMakePoint::call($ctx);
    ...
    public function test_make_point_rejects_an_operation_with_no_endpoints(): void
    {
        ...
        $ctx->op = new RealTimeBusDataOperation(['name' => 'load', 'entity' => 'x', 'points' => []]);
        [$point, $err] = RealTimeBusDataMakePoint::call($ctx);
        $this->assertNull($point);
    ...
    public function test_make_point_returns_the_single_point(): void
    {
        ...
        $ctx = self::ctx(['options' => ['allow' => ['op' => 'load,list,create,update,remove']]]);
        $ctx->op = new RealTimeBusDataOperation(['name' => 'load', 'entity' => 'x', 'points' => [$point]]);
        [$got, $err] = RealTimeBusDataMakePoint::call($ctx);
        $this->assertNull($err);
    ...
    {
        $preset = new RealTimeBusDataSpec(['method' => 'GET']);
        $ctx = self::ctx();
        $ctx->out['spec'] = $preset;
        [$got, $err] = RealTimeBusDataMakeSpec::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    public function test_make_response_guards_missing_spec_response_result(): void
    {
        $ctx = self::ctx([
            'spec' => null,
            'response' => new RealTimeBusDataResponse([]),
            'result' => new RealTimeBusDataResult([]),
        ]);
        [, $err] = RealTimeBusDataMakeResponse::call($ctx);
        $this->assertSame('response_no_spec', self::code($err));

        $ctx = self::ctx([
            'spec' => new RealTimeBusDataSpec([]),
            'response' => null,
            'result' => new RealTimeBusDataResult([]),
        ]);
        [, $err] = RealTimeBusDataMakeResponse::call($ctx);
    ...
    {
        $ctx = self::ctx([
            'spec' => new RealTimeBusDataSpec(['step' => 's']),
            'response' => new RealTimeBusDataResponse(self::resp(404, null, ['x-a' => '1'])),
            'result' => new RealTimeBusDataResult([]),
        ]);
        [, $err] = RealTimeBusDataMakeResponse::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_response_2xx_parses_the_body_and_marks_ok(): void
    {
        $ctx = self::ctx([
            'spec' => new RealTimeBusDataSpec(['step' => 's']),
            'response' => new RealTimeBusDataResponse(self::resp(200, ['v' => 1])),
            'result' => new RealTimeBusDataResult([]),
        ]);
        [, $err] = RealTimeBusDataMakeResponse::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_response_records_to_ctrl_explain_when_explain_is_on(): void
    {
        $ctx = self::ctx([
            'spec' => new RealTimeBusDataSpec(['step' => 's']),
            'response' => new RealTimeBusDataResponse(self::resp(200, ['v' => 2])),
            'result' => new RealTimeBusDataResult([]),
        ]);
        $ctx->ctrl->explain = ['on' => true];
        RealTimeBusDataMakeResponse::call($ctx);
    ...
    {
        $preset = new RealTimeBusDataResponse(self::resp(299));
        $ctx = self::ctx([
            'spec' => new RealTimeBusDataSpec([]),
            'response' => new RealTimeBusDataResponse([]),
            'result' => new RealTimeBusDataResult([]),
        ...
        [$got, $err] = RealTimeBusDataMakeResponse::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    {
        $ctx = self::ctx(['spec' => null, 'result' => new RealTimeBusDataResult([])]);
        [, $err] = RealTimeBusDataMakeResult::call($ctx);
        $this->assertSame('result_no_spec', self::code($err));

        $ctx = self::ctx(['spec' => new RealTimeBusDataSpec([]), 'result' => null]);
        [, $err] = RealTimeBusDataMakeResult::call($ctx);
    ...
    public function test_make_result_list_op_wraps_resdata_into_entity_instances(): void
    {
        ...
        $ctx = self::ctx([
            'entity' => $entity,
            'spec' => new RealTimeBusDataSpec(['step' => 's']),
            'result' => new RealTimeBusDataResult(['ok' => true, 'resdata' => [['a' => 1], ['a' => 2]]]),
        ]);
        $ctx->op = new RealTimeBusDataOperation(['name' => 'list', 'entity' => 'x']);
        [$result, $err] = RealTimeBusDataMakeResult::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_result_empty_list_yields_empty_resdata(): void
    {
        ...
        $ctx = self::ctx([
            'entity' => $entity,
            'spec' => new RealTimeBusDataSpec(['step' => 's']),
            'result' => new RealTimeBusDataResult(['ok' => true, 'resdata' => []]),
        ]);
        $ctx->op = new RealTimeBusDataOperation(['name' => 'list', 'entity' => 'x']);
        [$result, $err] = RealTimeBusDataMakeResult::call($ctx);
        $this->assertNull($err);
    ...
    {
        $preset = new RealTimeBusDataResult(['ok' => true]);
        $ctx = self::ctx([
            'spec' => new RealTimeBusDataSpec([]),
            'result' => new RealTimeBusDataResult([]),
        ]);
        $ctx->out['result'] = $preset;
        [$got, $err] = RealTimeBusDataMakeResult::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    public function test_make_request_guards_a_missing_spec(): void
    {
        ...
        [, $err] = RealTimeBusDataMakeRequest::call($ctx);
    ...
    {
        $utility = self::utility();
        $boom = new RealTimeBusDataError('boom', 'boom');
        $utility->fetcher = function (RealTimeBusDataContext $_c, string $_u, array $_f) use ($boom): array {
            return [null, $boom];
        };
        $ctx = self::ctx([
            'utility' => $utility,
            'spec' => new RealTimeBusDataSpec(['step' => 's', 'method' => 'GET', 'base' => 'http://h', 'parts' => ['a']]),
        ]);
        [$response, $err] = RealTimeBusDataMakeRequest::call($ctx);
        $this->assertNull($err);
    ...
    {
        $utility = self::utility();
        $utility->fetcher = function (RealTimeBusDataContext $_c, string $_u, array $_f): array {
            return [null, null];
        };
        $ctx = self::ctx([
            'utility' => $utility,
    ...
    public function test_make_request_a_normal_transport_response_is_wrapped(): void
    {
        ...
        [$response, $err] = RealTimeBusDataMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertInstanceOf(RealTimeBusDataResponse::class, $response);
    ...
    public function test_make_request_records_the_fetchdef_to_ctrl_explain(): void
    {
        ...
        $ctx->ctrl->explain = ['on' => true];
        RealTimeBusDataMakeRequest::call($ctx);
    ...
    {
        $utility = self::utility();
        $utility->make_fetch_def = function (RealTimeBusDataContext $c): array {
            return [null, $c->make_error('fetchdef_boom', 'boom')];
        };
        $ctx = self::ctx([
            'utility' => $utility,
            'spec' => new RealTimeBusDataSpec(['step' => 's', 'method' => 'GET']),
        ]);
        [$response, $err] = RealTimeBusDataMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertNotNull($response->err);
    ...
    {
        $preset = new RealTimeBusDataResponse(self::resp(201));
        $ctx = self::ctx(['spec' => new RealTimeBusDataSpec([])]);
        $ctx->out['request'] = $preset;
        [$got, $err] = RealTimeBusDataMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    {
        $ctx = self::ctx(['spec' => null]);
        [, $err] = RealTimeBusDataMakeFetchDef::call($ctx);
    ...
    public function test_make_fetch_def_serialises_body_and_inits_missing_result(): void
    {
        ...
        [$fetchdef, $err] = RealTimeBusDataMakeFetchDef::call($ctx);
        $this->assertNull($err);
    ...
    {
        $ctx = self::ctx(['result' => new RealTimeBusDataResult(['ok' => true, 'resdata' => 42])]);
        $this->assertSame(42, RealTimeBusDataDone::call($ctx));
    }

    public function test_done_raises_the_error_when_not_ok(): void
    {
        $ctx = self::ctx(['result' => new RealTimeBusDataResult(['ok' => false])]);
        $this->expectException(RealTimeBusDataError::class);
        RealTimeBusDataDone::call($ctx);
    }

    public function test_make_error_returns_resdata_when_ctrl_throw_is_false(): void
    {
        $ctx = self::ctx(['result' => new RealTimeBusDataResult(['ok' => false, 'resdata' => 'fallback'])]);
        $ctx->ctrl->throw_err = false;
        $this->assertSame('fallback', RealTimeBusDataMakeError::call($ctx, null));
    }
    ...
    public function test_make_error_records_to_ctrl_explain(): void
    {
        ...
        $ctx->ctrl->throw_err = false;
        $ctx->ctrl->explain = ['on' => true];
        RealTimeBusDataMakeError::call($ctx, null);
    ...
    public function test_feature_add_appends_in_call_order(): void
    {
        ...
        $a = new RealTimeBusDataBaseFeature();
        $b = new RealTimeBusDataBaseFeature();
        RealTimeBusDataFeatureAdd::call($ctx, $a);
        RealTimeBusDataFeatureAdd::call($ctx, $b);
        $this->assertSame([$a, $b], $client->features);
    }

    private static function named_feature(string $name): RealTimeBusDataBaseFeature
    {
        $f = new RealTimeBusDataBaseFeature();
        $f->name = $name;
        return $f;
    }
    ...
    public function test_feature_add_ordering_before_after_replace(): void
    {
        ...
        RealTimeBusDataFeatureAdd::call($ctx, self::named_feature('a'));
        RealTimeBusDataFeatureAdd::call($ctx, self::named_feature('b'));
        $this->assertSame(['a', 'b'], $names());

        $before = self::named_feature('z1');
        $before->_options = ['__before__' => 'b'];
        RealTimeBusDataFeatureAdd::call($ctx, $before);
        $this->assertSame(['a', 'z1', 'b'], $names());

        $after = self::named_feature('z2');
        $after->_options = ['__after__' => 'a'];
        RealTimeBusDataFeatureAdd::call($ctx, $after);
        $this->assertSame(['a', 'z2', 'z1', 'b'], $names());

        $replace = self::named_feature('z3');
        $replace->_options = ['__replace__' => 'z1'];
        RealTimeBusDataFeatureAdd::call($ctx, $replace);
        ...
        RealTimeBusDataFeatureAdd::call($ctx, $miss);
    ...
    private static function auth_ctx(array $options, ?array $headers): RealTimeBusDataContext
    {
        $client = new PlClient($options);
        return self::ctx([
            'client' => $client,
            'spec' => $headers === null ? null : new RealTimeBusDataSpec(['headers' => $headers]),
        ]);
    }
    ...
    public function test_prepare_auth_guards_a_missing_spec(): void
    {
        ...
        [, $err] = RealTimeBusDataPrepareAuth::call($ctx);
    ...
    public function test_prepare_auth_a_raw_apikey_goes_in_as_is(): void
    {
        ...
        RealTimeBusDataPrepareAuth::call($ctx);
    ...
    {
        $ctx = self::ctx([
            'response' => new RealTimeBusDataResponse(['headers' => null]),
            'result' => new RealTimeBusDataResult([]),
        ]);
        RealTimeBusDataResultHeaders::call($ctx);
    ...
    public function test_result_body_skips_parsing_when_the_body_is_absent(): void
    {
        ...
        RealTimeBusDataResultBody::call($ctx);
