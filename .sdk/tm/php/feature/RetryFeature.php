...
// RealTimeBusData SDK retry feature
...
class RealTimeBusDataRetryFeature extends RealTimeBusDataBaseFeature
{
    ...
    public function init(RealTimeBusDataContext $ctx, array $options): void
    {
        ...
        $utility->fetcher = function (RealTimeBusDataContext $ctx2, string $url, array $fetchdef) use ($inner): array {
            return $this->_with_retry($ctx2, $url, $fetchdef, $inner);
        };
    }

    private function _with_retry(RealTimeBusDataContext $ctx, string $url, array $fetchdef, callable $inner): array
    {
        $opts = $this->options ?? [];
    ...
    private function _track(RealTimeBusDataContext $_ctx, int $attempt, mixed $res, mixed $err, float $wait): void
    {
        $client = $this->client;
