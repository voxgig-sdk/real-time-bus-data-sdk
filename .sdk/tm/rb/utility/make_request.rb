...
module RealTimeBusDataUtilities
  MakeRequest = ->(ctx) {
    ...
    spec = ctx.spec
    utility = ctx.utility
    response = RealTimeBusDataResponse.new({})
    ...
    if fetch_err
      ...
      response = RealTimeBusDataResponse.new({ "err" => ctx.make_error("request_no_response", "response: undefined") })
    elsif fetched.is_a?(Hash)
      response = RealTimeBusDataResponse.new(fetched)
    else
