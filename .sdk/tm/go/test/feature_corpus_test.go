...
//
// NewRealTimeBusDataSDK, not TestSDK: the `test` feature is transport: 'base' and
// REPLACES the transport, so a client in test mode would shadow the script.
func fcClient(kase map[string]any) *sdk.RealTimeBusDataSDK {
	res, _ := kase["res"].([]any)
	opts := map[string]any{
	...
	return sdk.NewRealTimeBusDataSDK(opts)
}
...
func fcCandidates(client *sdk.RealTimeBusDataSDK) []fcOp {
	out := []fcOp{}
...
func fcInvoke(client *sdk.RealTimeBusDataSDK, op fcOp, ctrl map[string]any) error {
	cv := reflect.ValueOf(client)
	ct := cv.Type()
	mapType := reflect.TypeOf(map[string]any{})
...
// new go here.
func fcRecord(client *sdk.RealTimeBusDataSDK, name string) any {
			...
			for _, raw := range cases {
				...
				for _, rawstep := range steps {
					...
					if code, ok := wanterr.(string); ok {
						...
						// the word.
						got := ""
						var sdkerr *sdk.RealTimeBusDataError
