...
func TestEtaDirect(t *testing.T) {
	t.Run("direct-list-eta", func(t *testing.T) {
		setup := etaDirectSetup([]any{
		...
		if _shouldSkip, _reason := isControlSkipped("direct", "direct-list-eta", _mode); _shouldSkip {
			if _reason == "" {
		...
		if setup.live {
			for _, _liveKey := range []string{"route01", "service_type01", "stop01"} {
	...
	t.Run("direct-load-eta", func(t *testing.T) {
		setup := etaDirectSetup(map[string]any{"id": "direct01"})
		_mode := "unit"
		if setup.live {
			_mode = "live"
		}
		if _shouldSkip, _reason := isControlSkipped("direct", "direct-load-eta", _mode); _shouldSkip {
			if _reason == "" {
				_reason = "skipped via sdk-test-control.json"
			}
			t.Skip(_reason)
...
type etaDirectSetupResult struct {
...
func etaDirectSetup(mockres any) *etaDirectSetupResult {
	loadEnvLocal()

	calls := &[]map[string]any{}

	env := envOverride(map[string]any{
		"REAL_TIME_BUS_DATA_TEST_ETA_ENTID": map[string]any{},
	...
	if live {
		...
		idmap := map[string]any{}
		if entidRaw, ok := env["REAL_TIME_BUS_DATA_TEST_ETA_ENTID"]; ok {
		...
		return &etaDirectSetupResult{client: client, calls: calls, live: true, idmap: idmap}
	}
	...
	return &etaDirectSetupResult{client: client, calls: calls, live: false, idmap: map[string]any{}}
}
