...
func TestStopEntity(t *testing.T) {
	...
	t.Run("stream", func(t *testing.T) {
		...
		var seen []any
		for item := range base.Stop(nil).Stream("list", nil, nil) {
		...
		if hasStreaming {
			...
			var got []any
			for item := range streamSdk.Stop(nil).Stream("list", nil, nil) {
	...
	t.Run("basic", func(t *testing.T) {
		...
		if setup.syntheticOnly {
			t.Skip("live entity test uses synthetic IDs from fixture — set REAL_TIME_BUS_DATA_TEST_STOP_ENTID JSON to run live")
			return
		}
		client := setup.client
		...
		stopRef01Match := map[string]any{}

		stopRef01ListResult, err := stopRef01Ent.List(stopRef01Match, nil)
		if err != nil {
			t.Fatalf("list failed: %v", err)
		}
		_, stopRef01ListOk := stopRef01ListResult.([]any)
		if !stopRef01ListOk {
			t.Fatalf("expected list result to be an array, got %T", stopRef01ListResult)
		}
...
func stopBasicSetup(extra map[string]any) *entityTestSetup {
	...
	entidEnvRaw := os.Getenv("REAL_TIME_BUS_DATA_TEST_STOP_ENTID")
	...
	idmapResolved := core.ToMapAny(env["REAL_TIME_BUS_DATA_TEST_STOP_ENTID"])
