...
//     (\`new RealTimeBusDataSDK(...)\` / \`RealTimeBusDataSDK.test(...)\` become
//     \`RealTimeBusDataSDK.test({ entity: ... })\`), transpiled to JS, and run
...
const SDK_NAME = 'RealTimeBusDataSDK'
...
const TEST_SEED = {"entity":{"eta":{"test01":{"id":"test01"}},"route":{"test01":{"id":"test01"}},"route_stop":{"test01":{"id":"test01"}},"stop":{"test01":{"id":"test01"}}}}
...
// rewrite real client construction (\`new RealTimeBusDataSDK(...)\` and any
// \`RealTimeBusDataSDK.test(...)\`) to a seeded test-mode client, and give
...
function rewriteForRun(code: string): string {
  ...
  out = out.replace(/new\s+RealTimeBusDataSDK\s*\([^)]*\)/g, () => SEEDED_CTOR)
  out = out.replace(/RealTimeBusDataSDK\.test\s*\([^)]*\)/g, () => SEEDED_CTOR)
